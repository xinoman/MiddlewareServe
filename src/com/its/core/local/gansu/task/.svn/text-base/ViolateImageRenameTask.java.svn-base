...
/**
 * 创建日期 2013-7-16 下午10:18:07
 * @author GuoPing.Wu
 * Copyright: Xinoman Technologies CO.,LTD.
 */
public class ViolateImageRenameTask extends ATask {
	private static final Log log = LogFactory.getLog(ViolateImageRenameTask.class);
		...
		try {
			...
			for (int i = 0; i < files.length; i++) {	
				...
				if(file.isDirectory()){
					...
					String sbbh = content.substring(content.indexOf("deiveNo=")+8, content.indexOf(";directionNo"));
					...
					if(tpzs==3) {
						boolean moveSuccess1 = FileHelper.moveFile(new File(path+zjtp1), new File(this.getToDir() + "/" + targetName+"S31.JPG"));
						if (moveSuccess1) {
						...
						boolean moveSuccess2 = FileHelper.moveFile(new File(path+zjtp2), new File(this.getToDir() + "/" + targetName+"S32.JPG"));
						if (moveSuccess2) {
						...
						boolean moveSuccess3 = FileHelper.moveFile(new File(path+zjtp3), new File(this.getToDir() + "/" + targetName+"S33.JPG"));
						if (moveSuccess3) {
						...
						boolean moveSuccess1 = FileHelper.moveFile(new File(path+zjtp1), new File(this.getToDir() + "/" + targetName+"S21.JPG"));
						if (moveSuccess1) {
							log.warn("文件：" + zjtp1 + " 转移成功！");
						}
						else {
						...
						boolean moveSuccess2 = FileHelper.moveFile(new File(path+zjtp2), new File(this.getToDir() + "/" + targetName+"S22.JPG"));
						if (moveSuccess2) {
							log.warn("文件：" + zjtp2 + " 转移成功！");
						}
						else {
						...
						boolean moveSuccess1 = FileHelper.moveFile(new File(path+zjtp1), new File(this.getToDir() + "/" + targetName+"S11.JPG"));
						if (moveSuccess1) {
							log.warn("文件：" + zjtp1 + " 转移成功！");
						}
						else {
					...
					log.debug("本次共重命名违法图片数：[" + files.length +"]条 耗时："+(endTime-startTime)/1000+"秒！");
