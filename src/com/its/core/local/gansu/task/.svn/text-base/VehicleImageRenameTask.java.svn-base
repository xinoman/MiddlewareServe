		...
		try {
			...
			for (int i = 0; i < files.length; i++) {	
				...
				if(file.isDirectory()){
					...
					log.info("重命名图片文件名： " + targetName.toString());
					boolean moveSuccess = FileHelper.moveFile(new File(path+txtp), new File(this.getToDir() + "/" + targetName));
					if (moveSuccess) {
					...
					log.debug("本次共重命名卡口图片数：[" + files.length +"]条 耗时："+(endTime-startTime)/1000+"秒！");			
				}				
			}
		} catch (Exception ex) {
