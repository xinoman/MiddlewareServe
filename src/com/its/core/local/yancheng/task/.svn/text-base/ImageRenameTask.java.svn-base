 ...
 * 
 */
package com.its.core.local.yancheng.task;
		...
		try {				
			...
			for (int i = 0; i < files.length; i++) {	
				...
				if(file.isDirectory()){
					...
					String sbbh ="",jgsj="",cdbh="",hphm="",hpys="";
					try {
						String[] fileNameSplit = StringHelper.split(fileName, "_");
						sbbh = fileNameSplit[0];
						jgsj = fileNameSplit[2];
						cdbh = fileNameSplit[3];
						hphm = fileNameSplit[4];
						hpys = fileNameSplit[5].substring(0, fileNameSplit[5].indexOf("."));
						if(StringHelper.isEmpty(HPYS_MAP.get(hpys))) 
							hpys = "2";
						else
							hpys = HPYS_MAP.get(hpys);
					...
					StringBuffer targetName = new StringBuffer("X03R").append(
						sbbh).append("D1L").append(
						cdbh).append("I000V000").append("N").append(rad).append("T").append(jgsj);
