 ...
 * 
 */
package com.its.core.local.hezhou.filter;
...
//		}
	...
	protected void insertVehicleRecord(Connection conn,long key,RealtimeVehicleInfoBean realtimeVehicleInfoBean) throws Exception{
		...
		try{			
			...
			preStatement = conn.prepareStatement("select id from t_its_vehicle_record where feature_image_path = ?");
			preStatement.setString(1, realtimeVehicleInfoBean.getFeatureImagePath());
			...
			if(id==-1L){
				...
				log.debug("图片文件[" + realtimeVehicleInfoBean.getFeatureImagePath() + "] 重复上传！");
			}								
		}
		catch(Exception ex){
