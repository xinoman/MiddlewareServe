...
public class ExportToZlkjPdaserviceTask extends ATask {
	...
	private String xlh = "xdbgdfnjdfnzdtger6736jzdhtrtuej2";	
				...
				if(StringHelper.isNotEmpty(record.getImagePath1()) && StringHelper.isNotEmpty(record.getImagePath2()) && StringHelper.isNotEmpty(record.getImagePath3())){										
					...
					try{	
						...
						if(StringHelper.isNotEmpty(this.getImageSavePath())) {
							...
							if(record.getSpeed() > record.getLimitSpeed() && record.getSpeed() > 70) {
								log.info("号牌号码：" + record.getPlate()+ " 该条记录为区间测速，限速值为："+record.getLimitSpeed()+" 速度值为："+record.getSpeed());
								FileHelper.writeFile(fileBytes, filePath + dib.getThirdPartyDeviceInfoBean().getDeviceId()+"@"+DateHelper.dateToString(record.getViolateTime(),"yyyy年MM月dd日HH时mm分ss秒") + "@"+record.getPlateTypeId()+"@"+record.getPlate()+"@"+this.getFxjg()+"@"+record.getLimitSpeed()+"@"+record.getSpeed()+".JPG");
							} else {
	...
	private String getXmlDoc(ViolateRecordBean record,byte[] fileBytes) throws Exception {
		...
		xmlDoc.append("<root>");
			xmlDoc.append("<violation>");		
				...
				if(record.getSpeed() > record.getLimitSpeed() && record.getSpeed() > 70) {
					xmlDoc.append("<bzz>").append(record.getLimitSpeed()).append("</bzz>");
					xmlDoc.append("<scz>").append(record.getSpeed()).append("</scz>");
				} else {
	...
	public List<ViolateRecordBean> getUnexportRecord(Connection conn) throws Exception{
		...
		try{		
			...
			while(rs.next()){
				...
				record.setLimitSpeed(rs.getInt("limit_speed"));
