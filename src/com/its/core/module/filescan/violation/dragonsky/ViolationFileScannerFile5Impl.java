...
/**
 * 创建日期 2013-12-24 下午08:49:41
 * @author GuoPing.Wu
 * Copyright: Xinoman Technologies CO.,LTD.
 */
public class ViolationFileScannerFile5Impl extends AViolationFile5FileScanner {
	...
	protected ViolationInfoBean parseViolationInfo(String scanDir,File imageFile) throws Exception {
		...
		if(fileNameSplit[4].indexOf("未知") == -1) {
			bean.setPlateNo(fileNameSplit[4]);
		}
		...
		if(!dibMap.containsKey(fileNameSplit[3])){
			log.debug("未知的设备编号:" + fileNameSplit[3]);			
			return null;
		}
...
//		Map dibMap = DeviceInfoLoaderFactory.getInstance().getDeviceMap();
		...
		bean.setLine("0" + fileNameSplit[2].substring(1));
