...
/**
 * 创建日期 2013-12-24 下午08:48:45
 * @author GuoPing.Wu
 * Copyright: Xinoman Technologies CO.,LTD.
 */
public abstract class AViolationFile5FileScanner extends AFileScanner {
	private static final Log log = LogFactory.getLog(AViolationFile5FileScanner.class);
	...
	protected int cleanOldFile(File scanDir,long interval) {
		...
		for (int j = 0; j < len; j++) {
			if (files[j].isDirectory()){
				...
				*/
				int timeIndex = fileName.indexOf("-") + 1;
				if (timeIndex == -1){
				...
				Date date = null;
				try {
					timeStr = fileName.substring(1, 15);
