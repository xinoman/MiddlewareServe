...
import com.its.core.module.filescan.violation.ScannerParamBean;
...
/**
 * 创建日期 2012-12-13 下午05:21:10
 * @author GuoPing.Wu QQ:365175040
 * Copyright: ITS Technologies CO.,LTD.
 */
public abstract class AViolationFile2FileScanner extends AFileScanner {
	private static final Log log = LogFactory.getLog(AViolationFile2FileScanner.class);
...
//				if(StringHelper.isNotEmpty(targetId)) violationInfoBean.setDeviceId(targetId);
					...
					if((!isUseDbConn || hasConn) && scannerParam.isRequireMoveInvalidFile()){
						...
						try {
							...
							for(int i=0;i<violationInfoBean.getImageFiles().length;i++){
								String picFileName = violationInfoBean.getImageFiles()[i];
								if(StringHelper.isNotEmpty(picFileName)){
									sourceFileName = violationInfoBean.getFileDir() + "/" + picFileName;
									targetFileName = targetFileDir + "/" + picFileName;
				...
				if(scannerParam.isAddMd5Verify()){
					for(int i=0;i<violationInfoBean.getImageFiles().length;i++){
						String picFileName = violationInfoBean.getImageFiles()[i];
						if(StringHelper.isNotEmpty(picFileName)){
							String sourceFileName = violationInfoBean.getFileDir() + "/" + picFileName;
				...
				if ((System.currentTimeMillis() - file.lastModified()) > scannerParam.getMaxNoModifyTimeAllow() * 1000) {
					if(violationInfoBean!=null){
						for(int j=0;j<violationInfoBean.getImageFiles().length;j++){
							String picFileName = violationInfoBean.getImageFiles()[j];
							if(StringHelper.isNotEmpty(picFileName)){
								picFileName = violationInfoBean.getFileDir() + picFileName;								
								log.debug("删除文件：" + picFileName);
								try {
									FileHelper.delFile(new File(picFileName));
								} catch (Exception e) {
									log.error(e);
								}					
							}
	...
	/**
	 * 检测当前图片是否为重复记录（已存在于T_TEMP_PIC或T_FXC_RECORD中）
	 ...
	 * @return
	 */
	protected boolean statTempPicDay(Connection conn,ViolationInfoBean violationInfoBean){
...
//					log.debug("id = "+id+" updated!");
	...
	protected void processRepeatedData(ViolationInfoBean violationInfoBean){
		...
		for(int i=0;i<violationInfoBean.getImageFiles().length;i++){
			String picFileName = violationInfoBean.getImageFiles()[i];
			if(StringHelper.isNotEmpty(picFileName)){
				fileName = violationInfoBean.getFileDir() + "/" + picFileName;
	 ...
	 * @return
	 */
	protected String getVideoFileName(String jpgOriFileName,String scanDir,String deviceId, String yearMonthDayTime,String line, String videoId) {
		if (videoId == null || videoId.trim().equals(""))
			return null;
		File dirScan = new File(scanDir);
		String prefix = "R"+deviceId+"T"+yearMonthDayTime;
		String postfix = "ID" + videoId + scannerParam.getVideoFileExtension();
		...
		if (videoFileArr != null) {
			...
			/*
			for (int i = 0; i < len; i++) {
				videoFileName = videoFileArr[i].getName();
				if (videoFileName.indexOf("R" + deviceId) != -1)
		...
		if (idIndexOf != -1) {			
			if(!(scannerParam.isDirectDeleteVideo() && !scannerParam.isRequireVideo())){		
				...
				if (fileId != null && !fileId.trim().equals("")) {
					...
					String videoFileName = this.getVideoFileName(oriFileName,scanDir,deviceId,yearMonthDayTime, line, fileId);
				...
				if ((System.currentTimeMillis() - videoFile.lastModified()) > scannerParam.getMaxNoModifyTimeAllow() * 1000) {
					try {
						...
						for(int i=0;i<violationInfoBean.getImageFiles().length;i++){
							String picFileName = violationInfoBean.getImageFiles()[i];
							if(StringHelper.isNotEmpty(picFileName)){
								File imageFile = new File(scanDir + "/" + picFileName);
								
								String targetFileName = targetFileDir + "/" + picFileName;
								log.warn("视频文件大小小于设置值，将文件：" + picFileName + " 转移到：" + targetFileName);
								boolean moveSuccess = FileHelper.moveFile(imageFile, new File(targetFileName));
								if (moveSuccess)
									log.warn("文件：" + picFileName + " 转移成功！");
								else
									log.warn("文件：" + picFileName + " 转移失败，文件可能正在使用中！");								
							}
						}
	...
	protected void createMark(ViolationInfoBean violationInfoBean){
		...
		for(int i=0;i<violationInfoBean.getImageFiles().length;i++){
			String picFileName = violationInfoBean.getImageFiles()[i];
			if(StringHelper.isNotEmpty(picFileName)){
				String fullPicName = scanDir + "/" + picFileName;
	...
	public boolean backupViolationInfoFile(ViolationInfoBean violationInfoBean) {
		boolean result = true;
		try {		
			String backDir = scannerParam.getBackupDir() + "/" + violationInfoBean.getDeviceId() + "/" + DateHelper.dateToString(violationInfoBean.getViolateTime(), "yyyyMMdd");
			
			for(int i=0;i<violationInfoBean.getImageFiles().length;i++){
				String picFileName = violationInfoBean.getImageFiles()[i];
				if(StringHelper.isNotEmpty(picFileName)){				
					File imageFile = new File(violationInfoBean.getFileDir() + "/" + picFileName);
					
					FileHelper.createDir(backDir);				
					File targetFile = new File(backDir + "/" + picFileName);
	...
	public boolean buildBackupData(ViolationInfoBean violationInfoBean) {
		...
		try{
			...
			for(;i<violationInfoBean.getImageFiles().length;i++){
				String picFileName = violationInfoBean.getImageFiles()[i];
				backupDataBuff.append(picFileName);
				backupDataBuff.append(",");												
			}
			for(;i<4;i++){
	...
	public int restoreBackupFile(Connection conn,boolean isUseDbConn,File backupFile) throws Exception{
		...
		try{
			...
			while(true){
				...
				String fileDir = scannerParam.getBackupDir() + "/" + deviceId + "/" + DateHelper.dateToString(violationInfoBean.getViolateTime(), "yyyyMMdd") + "/";
				...
				if(!isRepeated){
					...
					if(processResult != 0){
						...
						this.statTempPicDay(conn, violationInfoBean);
		...
		for (int j = 0; j < len; j++) {
			if (files[j].isDirectory()){
				...
				*/
				...
				Date date = null;
				try {
					timeStr = "2012" + StringHelper.replace(fileName.substring(timeIndex, timeIndex+11), "C", "");
