...
/**
 * 创建日期 2013-4-17 上午10:59:54
 * @author GuoPing.Wu QQ:365175040
 * Copyright: Xinoman Technologies CO.,LTD.
 */
public class VechileRecordHistoryAndTablespaceBackupTask extends AFixedHourTask {
	private static final Log log = LogFactory.getLog(VechileRecordHistoryAndTablespaceBackupTask.class);
	...
	private String createHistoryTablespace = null;
	...
	public void configureSpecificallyProperties(XMLProperties props,String propertiesPrefix, int no) {
		...
		this.createHistoryTablespace = props.getProperty(propertiesPrefix,no,"sql."+dbType+".create_history.tablespace");
	...
	private void createtVehicleRecordTableAndIndex(Connection conn,Date startTime) throws Exception{
		...
		try{		
			...
			//检查表空间文件是否存在
			String tablespace = StringHelper.replace(this.getCreateHistoryTablespace(), "${YYYYMMDD}", yearMonthDay);
			String filePath = tablespace.substring(tablespace.indexOf("'")+1, tablespace.lastIndexOf("'"));
			log.debug(filePath);
			boolean tablespaceExist = false;
			if(new File(filePath).exists()) {
				tablespaceExist = true;
			}
			if(!tablespaceExist) {
				//创建表空间DBF文件目录
                String dbfPath = tablespace.substring(tablespace.indexOf("'")+1, tablespace.lastIndexOf("\\"));
                boolean result = FileHelper.createDir(dbfPath);
                log.debug("创建表空间DBF文件目录"+filePath+"状态为："+result+"(true:成功，false:失败)");
				//创建表空间
				String tablespaceSql = StringHelper.replace(this.getCreateHistoryTablespace(), "${YYYYMMDD}", yearMonthDay);
				log.debug(tablespaceSql);
				preStatement = conn.prepareStatement(tablespaceSql);
				preStatement.executeUpdate();
				DatabaseHelper.close(null,preStatement);
			}
	...
	/**
	 * @return the createHistoryTablespace
	 */
	public String getCreateHistoryTablespace() {
		return createHistoryTablespace;
	}

	/**
	 * @param createHistoryTablespace the createHistoryTablespace to set
	 */
	public void setCreateHistoryTablespace(String createHistoryTablespace) {
		this.createHistoryTablespace = createHistoryTablespace;
	}
